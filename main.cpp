...
testProduct(void) {
    ...
    float s2 = product(v2);
    ensure(floatEqual(s2, 120), "product test error 2");
}
...
testAbs1(void) {
    float s1 = 1.1;
    ensure(floatEqual(s1, 1.1), "abs test error 1");
    float s2 = -1.1;
    ensure(floatEqual(s2, 1.1), "abs test error 2");
}
// 作业 3
// 参数是一个只包含数字的数组
// 求 数组 中所有数字的平均数
...
void
testAverage(void) {
    vector<float> v1 = {1, -1, 3.4, -3.4};
    float s1 = average(v1);
    ensure(floatEqual(s1, 0), "average test error 1");

    vector<float> v2 = {1, 2, 3, 4};
    float s2 = 2.5;
    ensure(floatEqual(s2, 2.5), "average test error 2");
}
// 作业 4
// 参数是一个只包含数字的 数组
// 求 数组 中最小的数字
...
void
testMin1(void) {
    vector<float> s1 = {2.3, 2, -1, 1};
    float v1 = min1(s1);
    ensure(floatEqual(v1, -1), "min test error 1");

    vector<float> s2 = {2.3, 2, 1, 1};
    float v2 = min1(s2);
    ensure(floatEqual(v2, 1), "min test error 2");
}
...
testSum1(void) {
    int v1 = sum1(3);
    ensure(v1 == 1 - 2 + 3, "sum1 test error 1");

    int v2 = sum1(4);
    ensure(v2 == 1 - 2 + 3 - 4, "sum1 test error 1");
}
...
testSum2(void) {
    int v1 = sum2(3);
    ensure(v1 == 1 + 2 - 3, "sum2 test error 1");

    int v2 = sum2(4);
    ensure(v2 == 1 + 2 - 3 + 4, "sum2 test error 1");
}
...
testFac(void) {
    int v1 = fac(3);
    ensure(v1 == 1 * 2 * 3, "fac test error 1");

    int v2 = fac(4);
    ensure(v2 == 1 * 2 * 3 * 4, "fac test error 1");
}
